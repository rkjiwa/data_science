...
from dlgo.hexapawn.types import Player, Point
...
class GameState:
	...
	def hex_cross(self, player):
		if player == Player.x:
			if self.board.get(Point(3,1)) == Player.x or \
				self.board.get(Point(3,2)) == Player.x or \
				self.board.get(Point(3,3)) == Player.x:
				return True
		if player == Player.o:
			if self.board.get(Point(1,1)) == Player.o or \
				self.board.get(Point(1,2)) == Player.o or \
				self.board.get(Point(1,3)) == Player.o:
				return True
		return False
